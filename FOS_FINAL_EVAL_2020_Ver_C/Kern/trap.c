...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	struct Frame_Info * faultFrame = NULL;
	allocate_frame(&faultFrame);
	map_frame(curenv->env_page_directory,faultFrame,(void*) fault_va, PERM_WRITEABLE|PERM_USER);
	int flag = 0;
	if(pf_read_env_page(curenv,(void*) fault_va) ==  E_PAGE_NOT_EXIST_IN_PF){
		for(uint32 i = USTACKBOTTOM; i<= USTACKTOP; i+= PAGE_SIZE){
			if(PDX(fault_va) == PDX(i)){
		...
		if(flag == 0){
			panic("the faulted address is wrong address !!!");
		}
	}
